#!/usr/bin/env python3
"""
Compute accuracy for FOLIO DC neuro-symbolic runs with deterministic majority vote,
focusing only on conclusions that contain operators.

Rules:
- Only evaluate examples from the specified operator indices list
...
Examples
--------
python overall_accuracy_dcneurosym.py \
  --gens /app/linc/outputs/Mistral-7B-v0.1_folio-dcneurosymbolic-1shot_generations_prc.json \
  --refs /app/linc/outputs/Mistral-7B-v0.1_folio-dcneurosymbolic-1shot_references.json \
  --save-csv /app/linc/dc_analysis/dcneuro2_analysis/dcneurosym_operator_predictions.csv \
  --save-json /app/linc/dc_analysis/dcneuro2_analysis/dcneurosym_operator_summary.json
"""
...
DEFAULT_GENS = DEFAULT_BASE / "Mistral-7B-v0.1_folio-dcneurosymbolic-1shot_generations_prc.json"
DEFAULT_REFS = DEFAULT_BASE / "Mistral-7B-v0.1_folio-dcneurosymbolic-1shot_references.json"
DEFAULT_CSV = Path("/app/linc/dc_analysis/dcneuro2_analysis/dcneurosym_operator_predictions.csv")
DEFAULT_JSON = Path("/app/linc/dc_analysis/dcneuro2_analysis/dcneurosym_operator_summary.json")
DEFAULT_FIG = Path("/app/linc/dc_analysis/dcneuro2_analysis/dcneurosym_operator_confusion_matrix.png")

# The list of operator indices to filter by (provided in the request)
OPERATOR_INDICES = [
  0, 3, 4, 9, 10, 11, 12, 13, 15, 18, 25, 30, 31, 34, 36, 38, 40, 41, 45, 48, 49, 50, 53, 58, 
  60, 61, 62, 63, 64, 71, 74, 75, 78, 82, 84, 86, 90, 93, 94, 95, 97, 101, 103, 104, 112, 116, 
  118, 124, 127, 128, 129, 130, 137, 138, 140, 148, 150, 151, 156, 162, 163, 165, 166, 167, 
  169, 171, 173, 174, 177, 179, 180, 181
]
...
def plot_confusion(references: List[str],
    ...
    ax.set_title("Confusion Matrix (D&C LINC Operator-only Examples)")
    fig.tight_layout()
    if save_path:
...
def main():
    ...
    print(f"Using {len(OPERATOR_INDICES)} operator indices")
    ...
    print("\nOperator-only Accuracy Analysis (DC-LINC)")
    print("---------------------------------------")
    print(f"Count:    {n}")
