...
Default input files match the ones you uploaded to /mnt/data.
You can override them via CLI flags.

Examples
--------
python overall_accuracy_neurosym_fixed.py \
  ...
  --save-csv /app/linc/dc_analysis/neurosym_predictions.csv \
  --save-json /app/linc/dc_analysis/neurosym_summary.json

Section 5.2.1 Overall Accuracy (for dissertation integration)
-----------------------------------------------------------
- Report pass@1 majority accuracy (≈58% for baseline).
- Include a table with: accuracy, uncertainty rate, and precision/recall/F1 per class.
"""
...
DEFAULT_CSV = Path("/app/linc/dc_analysis/dcneuro1_analysis/dcneurosym_predictions.csv")
DEFAULT_JSON = Path("/app/linc/dc_analysis/dcneuro1_analysis/dcneurosym_summary.json")
DEFAULT_FIG = Path("/app/linc/dc_analysis/dcneuro1_analysis/dcneurosym_confusion_matrix.png")
...
def plot_confusion(references: List[str],
    ...
    ax.set_title("Confusion Matrix (D&C LINC)")
    fig.tight_layout()
    if save_path:
...
def main():
    ...
    references: List[str] = load_json(args.refs)
    ...
    predictions: List[str] = [majority_vote_ignore_error(row) for row in generations]
    ...
    if args.save_csv:
        save_csv(args.save_csv, references, predictions)
    if args.save_json:
        summary = {"count": n, "correct": correct, "accuracy": accuracy, "report": report}
    ...
    if args.save_fig:
        ...
        plot_confusion(references=references, predictions=predictions, save_path=args.save_fig, 
